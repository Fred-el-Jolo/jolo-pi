...
- **Round-2 fixes (2026-08-27, full trail in `notes/specloop-audit-log.md`):** startup probe no longer latches off on transient failures (`session_start` re-probes and recovers); confirmation second evidence bar — ≥3 distinct `learned_days` promotes for resumed sessions; dedup gate re-measured → 0.75 (family ≥0.729 vs unrelated ≤0.728); new `mem dedup` repair command (dry-run/`--apply`, round-based cascade, `dedup`/`repair` audit events, `Memory.delete_node`); merge audit now carries `confirmed_by_count`/`learned_days_count`. Known accepted risks: permissive merge arbiter near threshold (over-merge incident surgically split, see log R2-6); slight API-embedding drift fresh-vs-stored (R2-8).
